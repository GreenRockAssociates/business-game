...
import {PlayerIdInternalResponseDto} from "../dto/player-id-internal-response.dto";
...
export class LauncherService {
    ...
    async translateUserIdToPlayerIdForGame(gameId: string, headers: IncomingHttpHeaders): Promise<string> {
        const requestUrl = `${process.env.BASE_SERVER_URL}${process.env.LAUNCHER_SERVICE_PREFIX}/games/${gameId}/players/engine-id`
        const rawResponse = await this.executeGetRequest(requestUrl, headers);
        const validatedResponse = await this.castAndValidateResponse(rawResponse, PlayerIdInternalResponseDto);
